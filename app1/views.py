        ...
        try:
                if request.method == 'GET':
                    ...
                    if banner_list:
                            ...
                            "linkUrl": '',
                            "paixu": 0,
                            "picUrl": app.host+"/media/"+str(each_banner.display_pic),
                    ...
                    "data": [
                        {
                            ...
                            "paixu": 0,
                            "pic": app.host+"/media/"+str(each_goods.display_pic),
                            "recommendStatus": 0,
                            "recommendStatusStr": "",
                            "shopId": 0,
                            "status": 0,
           ...
           data = {
                ...
                "data": {
                    ...
                    "basicInfo": {
                        ...
                        "paixu": 0,
                        "pic": app.host+"/media/"+str(goods.display_pic),
                        "recommendStatus": 0,
                        "recommendStatusStr": "",
                        "shopId": 0,
                        "status": 0,
...
def articles(request,sub_domain='test'):
    try:
        app = AppConfig.objects.get(sub_domain='test')
        articles = Article.objects.filter(app=app)
        print len(articles)
        data = {
            'code':0,
            'data':
              {'items':[
              {
                ...
                'goods_id':each.id,
                'title':each.title,
                'summary':each.content,
                'catch_title':each.title,
                'cover':app.host+'/media/'+str(each.cover),
                'update_at':each.date.strftime('%Y-%m-%d %H:%M:%S'),
                'published_at':each.date.strftime('%Y-%m-%d %H:%M:%S'),
                'create_at':each.date.strftime('%Y-%m-%d %H:%M:%S'),
